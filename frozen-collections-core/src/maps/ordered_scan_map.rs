...
impl<Q, K, V> IndexMut<&Q> for OrderedScanMap<K, V>
where
    K: Borrow<Q>,
    Q: ?Sized + Ord,
{
...
mod tests {
    ...
    #[test]
    fn test_ordered_scan_map() {
        let map = OrderedScanMap::new(vec![(1, 1), (2, 2), (3, 3)]);
        ...
        let map = OrderedScanMap::new(vec![]);
        let reference = StdHashMap::from([]);
        let other = StdHashMap::from([(1, 1), (2, 2), (3, 3), (4, 4)]);
        test_map_trait_impl(&map, &reference, &other);

        let map = OrderedScanMap::new(vec![(1, 1), (2, 2), (3, 3), (1, 4)]);
    ...
    fn test_get_many_mut() {
        let mut map = OrderedScanMap::new(vec![(1, 1), (2, 2), (3, 3)]);
    ...
    fn test_into_iter() {
        ...
        let map = &mut OrderedScanMap::new(vec![(1, 1), (2, 2), (3, 3)]);
        let into_map: StdHashMap<_, _> = map.into_iter().map(|(k, v)| (*k, *v)).collect();
        assert_eq!(map, &into_map);
    }
}
