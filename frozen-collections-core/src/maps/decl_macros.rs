...
macro_rules! dense_sequence_lookup_core {
    () => {
        #[inline]
        #[must_use]
        ...
        where
            ...
            Q: Sequence,
        {
            if let Some(index) = Q::offset(self.min.borrow(), self.max.borrow(), key) {
        ...
        get_many_mut_fn!(Sequence);
        contains_key_fn!(Sequence);
    };
}
...
macro_rules! binary_search_core {
    () => {
        ...
        where
            ...
            Q: ?Sized + Ord,
        {
            binary_search_by_key(&self.entries, key, |entry| entry.0.borrow())
...
macro_rules! sparse_sequence_lookup_core {
    () => {
        #[inline]
        #[must_use]
        pub fn get<Q>(&self, key: &Q) -> Option<&V>
        where
            ...
            Q: Sequence,
        {
            if let Some(index_in_lookup) = Q::offset(self.min.borrow(), self.max.borrow(), key) {
...
pub(crate) use sparse_sequence_lookup_core;
