...
pub fn process(payload: Payload, emitter: CollectionEmitter, macro_kind: MacroKind) -> syn::Result<TokenStream> {
...
fn handle_literal_string_keys(emitter: CollectionEmitter, entries: Vec<Entry>) -> syn::Result<TokenStream> {
    ...
    for entry in entries {
        ...
        if entry.value.is_some() {
            coll_entries.push(CollectionEntry::map_entry(ls.value().to_string(), entry.key, entry.value.unwrap()));
        } else {
            coll_entries.push(CollectionEntry::set_entry(ls.value().to_string(), entry.key));
        }
    }
