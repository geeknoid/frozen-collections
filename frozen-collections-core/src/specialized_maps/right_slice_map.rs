    ...
    pub fn with_hasher(
        ...
        let codes = payload.iter().map(|entry| {
            ...
            if key.len() >= range.end {
                key.hash_range(&bh, key.len() - range.start..key.len() - range.end)
            } else {
                0
            }
        });
        ...
        Ok(Self {
            table: HashTable::new(payload, code_analysis.num_hash_slots, |k| {
                k.hash_range(&bh, k.len() - range.start..k.len() - range.end)
            })?,
            bh,
            range,
        })
...
impl<K, V, S, BH> Default for RightSliceMap<K, V, S, BH>
where
    S: PrimInt + Unsigned,
...
impl<'a, K, V, S, BH> IntoIterator for &'a mut RightSliceMap<K, V, S, BH> {
...
impl<K, V, S, BH> IntoIterator for RightSliceMap<K, V, S, BH> {
    type Item = (K, V);
