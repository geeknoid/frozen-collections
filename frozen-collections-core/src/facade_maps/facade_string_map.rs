...
#[derive(Clone)]
enum MapTypes<V, BH> {
    LeftRange(HashMap<String, V, LeftRangeHasher<BH>>),
    RightRange(HashMap<String, V, RightRangeHasher<BH>>),
    Hash(HashMap<String, V, BridgeHasher<BH>>),
}
    ...
    BH: BuildHasher,
{
    ...
    fn new_from_string(mut entries: Vec<(String, V)>, bh: BH) -> Self {
    ...
    #[must_use]
    #[allow(clippy::missing_panics_doc)]
    pub fn new(entries: Vec<(&str, V)>, bh: BH) -> Self {
