...
use crate::maps::decl_macros::{
    ...
    len_trait_funcs, map_iteration_trait_funcs, map_query_trait_funcs, partial_eq_trait_funcs,
};
use crate::maps::{
    ...
    K: Scalar + Eq,
{
    ...
    unsafe fn get_disjoint_unchecked_mut<const N: usize>(&mut self, keys: [&K; N]) -> [Option<&mut V>; N] {
        match &mut self.map_impl {
            ...
            // SAFETY: The caller must ensure that the keys are disjoint.
            MapTypes::Dense(m) => unsafe { m.get_disjoint_unchecked_mut(keys) },

            // SAFETY: The caller must ensure that the keys are disjoint.
            MapTypes::Sparse(m) => unsafe { m.get_disjoint_unchecked_mut(keys) },
        }
    }
}

impl<K, V, Q> MapQuery<K, V, Q> for FzScalarMap<K, V>
where
    Q: Scalar + Comparable<K>,
{
