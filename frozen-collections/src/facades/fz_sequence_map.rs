...
use frozen_collections_core::maps::{
    DenseSequenceLookupMap, HashMap, IntoIter, IntoKeys, IntoValues, Iter, IterMut, Keys,
    SparseSequenceLookupMap, Values, ValuesMut,
};
use frozen_collections_core::traits::{Hasher, LargeCollection, Len, Map, MapIterator, Sequence};
...
enum MapTypes<K, V> {
    ...
    Dense(DenseSequenceLookupMap<K, V>),
    Sparse(SparseSequenceLookupMap<K, V, LargeCollection>),
}
...
/// #
/// let map = FzSequenceMap::from([(1, "One"), (2, "Two"), (3, "Three")]);
///
/// assert_eq!(3, map.len());
...
pub struct FzSequenceMap<K, V> {
    map_impl: MapTypes<K, V>,
}

impl<K, V> FzSequenceMap<K, V>
where
    K: Sequence,
{
    ...
    /// # use frozen_collections::Len;
    /// #
    /// let map = FzSequenceMap::new(vec![(1, 2), (3, 4)]);
    ///
    /// assert_eq!(2, map.len());
    ...
    pub fn new(mut entries: Vec<(K, V)>) -> Self {
        ...
        Self {
            map_impl: match analyze_sequence_keys(entries.iter().map(|x| x.0)) {
                SequenceKeyAnalysisResult::DenseRange => {
                    MapTypes::Dense(DenseSequenceLookupMap::new_internal(entries))
                }
                SequenceKeyAnalysisResult::SparseRange => {
                    MapTypes::Sparse(SparseSequenceLookupMap::new_internal(entries))
                }
                SequenceKeyAnalysisResult::General => {
                    let h = PassthroughHasher::new();
                    let analysis = analyze_hash_codes(entries.iter().map(|x| h.hash(&x.0)));
                    MapTypes::Hash(
                        HashMap::with_hasher(entries, analysis.num_hash_slots, h).unwrap(),
                    )
                }
            },
        }
...
impl<K, V> FzSequenceMap<K, V> {
    ...
    /// # use frozen_collections::FzSequenceMap;
    /// #
    /// let map = FzSequenceMap::from([(1, "a")]);
    ///
    ...
    /// # use frozen_collections::FzSequenceMap;
    /// #
    /// let mut map = FzSequenceMap::from([(1, "a")]);
    ///
...
impl<K, V> Len for FzSequenceMap<K, V> {
    fn len(&self) -> usize {
        match &self.map_impl {
...
impl<K, V> From<Vec<(K, V)>> for FzSequenceMap<K, V>
where
    K: Sequence,
{
...
impl<K, V, const N: usize> From<[(K, V); N]> for FzSequenceMap<K, V>
where
    K: Sequence,
{
...
impl<K, V> FromIterator<(K, V)> for FzSequenceMap<K, V>
where
    K: Sequence,
{
...
impl<K, V> Index<&K> for FzSequenceMap<K, V>
where
    K: Sequence,
{
    type Output = V;
...
impl<K, V> IndexMut<&K> for FzSequenceMap<K, V>
where
    K: Sequence,
{
...
impl<K, V> Default for FzSequenceMap<K, V>
where
    K: Sequence,
{
    fn default() -> Self {
        Self {
            map_impl: MapTypes::Dense(DenseSequenceLookupMap::default()),
        }
    }
}

impl<K, V> Debug for FzSequenceMap<K, V>
where
    K: Debug,
    V: Debug,
{
...
impl<K, V, MT> PartialEq<MT> for FzSequenceMap<K, V>
where
    K: Sequence,
    V: PartialEq,
    MT: Map<K, V>,
...
impl<K, V> Eq for FzSequenceMap<K, V>
where
    K: Sequence,
    V: Eq,
{
...
impl<'a, K, V> IntoIterator for &'a FzSequenceMap<K, V> {
    type Item = (&'a K, &'a V);
...
impl<'a, K, V> IntoIterator for &'a mut FzSequenceMap<K, V> {
...
impl<K, V> IntoIterator for FzSequenceMap<K, V> {
    type Item = (K, V);
...
impl<K, V> MapIterator<K, V> for FzSequenceMap<K, V> {
...
impl<K, V> Map<K, V> for FzSequenceMap<K, V>
where
    K: Sequence,
{
    #[inline]
