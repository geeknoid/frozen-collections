...
use frozen_collections_core::traits::{Hasher, LargeCollection, Len, Map, MapIterator};
    ...
    pub fn with_hasher(entries: Vec<(K, V)>, bh: BH) -> Self {
        Self {
            map_impl: if entries.len() < 3 {
                ...
                MapTypes::Hash(HashMap::with_hasher(entries, analysis.num_hash_slots, h).unwrap())
            },
        }
    }
}
...
impl<K, V, BH> Index<&K> for FzHashMap<K, V, BH>
where
    K: Hash + Eq,
    BH: BuildHasher,
{
...
impl<K, V, BH> IndexMut<&K> for FzHashMap<K, V, BH>
where
    K: Hash + Eq,
    BH: BuildHasher,
{
