...
use frozen_collections_core::analyzers::{
    analyze_hash_codes, analyze_sequence_keys, SequenceKeyAnalysisResult,
};
use frozen_collections_core::hashers::PassthroughHasher;
use frozen_collections_core::sets::{
    DenseSequenceLookupSet, HashSet, IntoIter, Iter, SparseSequenceLookupSet,
};
use frozen_collections_core::traits::{Hasher, LargeCollection, Len, Sequence, Set, SetIterator};
...
enum SetTypes<T> {
    ...
    Dense(DenseSequenceLookupSet<T>),
    Sparse(SparseSequenceLookupSet<T, LargeCollection>),
}
...
///
/// ```
/// # use frozen_collections::FzSequenceSet;
/// # use frozen_collections::Len;
/// #
/// let set = FzSequenceSet::from([1, 2, 3]);
///
/// assert_eq!(3, set.len());
...
pub struct FzSequenceSet<T> {
    set_impl: SetTypes<T>,
}

impl<T> FzSequenceSet<T>
where
    T: Sequence,
{
    ...
    /// # use frozen_collections::Len;
    /// #
    /// let set = FzSequenceSet::new(vec![1, 2, 3]);
    ///
    /// assert_eq!(set.len(), 3);
    /// assert!(set.contains(&1));
    /// ```
    ...
    pub fn new(mut entries: Vec<T>) -> Self {
        ...
        Self {
            set_impl: match analyze_sequence_keys(entries.iter().copied()) {
                SequenceKeyAnalysisResult::DenseRange => {
                    SetTypes::Dense(DenseSequenceLookupSet::new_internal(entries))
                }
                SequenceKeyAnalysisResult::SparseRange => {
                    SetTypes::Sparse(SparseSequenceLookupSet::new_internal(entries))
                }
                SequenceKeyAnalysisResult::General => {
                    ...
                    SetTypes::Hash(
                        HashSet::with_hasher(
                            entries,
                            analysis.num_hash_slots,
                            PassthroughHasher::new(),
                        )
                        .unwrap(),
                    )
                }
...
impl<T> From<Vec<T>> for FzSequenceSet<T>
where
    T: Sequence,
{
...
impl<T, const N: usize> From<[T; N]> for FzSequenceSet<T>
where
    T: Sequence,
{
...
impl<T> FromIterator<T> for FzSequenceSet<T>
where
    T: Sequence,
{
...
impl<T> Len for FzSequenceSet<T> {
    fn len(&self) -> usize {
        match &self.set_impl {
...
impl<T> Default for FzSequenceSet<T>
where
    T: Sequence,
{
    fn default() -> Self {
        Self {
            set_impl: SetTypes::Dense(DenseSequenceLookupSet::default()),
        }
    }
}

impl<T> Debug for FzSequenceSet<T>
where
    T: Debug,
{
...
impl<T, ST> PartialEq<ST> for FzSequenceSet<T>
where
    T: Sequence,
    ST: Set<T>,
{
...
impl<T> Eq for FzSequenceSet<T> where T: Sequence {}

impl<T, ST> BitOr<&ST> for &FzSequenceSet<T>
where
    T: Sequence + Hash,
    ST: Set<T>,
{
...
impl<T, ST> BitAnd<&ST> for &FzSequenceSet<T>
where
    T: Sequence + Hash,
    ST: Set<T>,
{
...
impl<T, ST> BitXor<&ST> for &FzSequenceSet<T>
where
    T: Sequence + Hash,
    ST: Set<T>,
{
...
impl<T, ST> Sub<&ST> for &FzSequenceSet<T>
where
    T: Sequence + Hash,
    ST: Set<T>,
{
...
impl<T> IntoIterator for FzSequenceSet<T> {
    type Item = T;
...
impl<'a, T> IntoIterator for &'a FzSequenceSet<T> {
    type Item = &'a T;
...
impl<T> SetIterator<T> for FzSequenceSet<T> {
...
impl<T> Set<T> for FzSequenceSet<T>
where
    T: Sequence,
{
...
mod tests {
    ...
    fn misc(entries: Vec<i32>) {
        let set = FzSequenceSet::new(entries.clone());
        ...
        let set3 = FzSequenceSet::<i32>::from_iter(entries.clone());
        assert_eq!(set, set3);

        let set4 = FzSequenceSet::<i32>::from(entries);
        assert_eq!(set, set4);

        let set5 = FzSequenceSet::<i32>::from([0, 1, 2, 3]);
    ...
    fn bits(entries: Vec<i32>) {
        let fs1 = FzSequenceSet::new(entries.clone());
        let fs2 = FzSequenceSet::new(entries.iter().map(|x| x + 1).collect());
    ...
    #[test]
    fn default() {
        let fs = FzSequenceSet::<i32>::default();
        assert_eq!(0, fs.len());
    }
}
