...
use frozen_collections_core::analyzers::analyze_hash_codes;
...
///
/// If you can't use the `frozen_set!`, but you know at compile time that your values are integers or strings, you should use the
/// [`crate::FzSequenceSet`] and [`crate::FzStringSet`] types respectively for better performance.
///
/// # Examples
///
    ...
    pub fn with_hasher(entries: Vec<T>, bh: BH) -> Self {
        Self {
            set_impl: if entries.len() <= 3 {
                ...
                SetTypes::Hash(HashSet::with_hasher(entries, analysis.num_hash_slots, h).unwrap())
            },
        }
    }
}
...
mod tests {
    ...
    fn iter(entries: Vec<i32>) {
        ...
        let hs = HashSet::<i32, LargeCollection, _>::with_hasher(
            HashSet::<&i32, LargeCollection, _>::with_hasher(
                set.iter().collect(),
                128,
