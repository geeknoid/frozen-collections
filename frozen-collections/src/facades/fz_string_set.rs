...
/// [`fz_string_set!`](crate::fz_string_set) macro rather than this type. This will result in considerably
/// better performance.
///
/// # Examples
///
            ...
            set_impl: {
                match analyze_slice_keys(entries.iter().map(String::as_bytes), &bh) {
                    ...
                    SliceKeyAnalysisResult::LeftHandSubslice(range) => {
                        ...
                        SetTypes::LeftRange(HashSet::new(entries, h).unwrap())
                    }

                    SliceKeyAnalysisResult::RightHandSubslice(range) => {
                        let h = RightRangeHasher::new(bh, range);
                        SetTypes::RightRange(HashSet::new(entries, h).unwrap())
                    }
                }
            },
        }
