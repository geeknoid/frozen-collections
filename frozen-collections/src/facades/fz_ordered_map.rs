...
/// An ordered map optimized for fast read access.
///
/// A frozen map differs from the traditional [`HashMap`](std::collections::HashMap) type in three
/// key ways. First, creating
...
///
/// A [`FzOrderedMap`] requires that the elements
/// implement the [`Eq`] and [`Ord`] traits. This can frequently be achieved by
/// using `#[derive(PartialEq, Eq, PartialOrd, Ord)]`.
///
...
/// be encapsulated to the `FzOrderedMap` that observed the logic error and not
...
/// [`fz_ordered_map!`](crate::fz_ordered_map!) macro rather than this type. This will result in considerably
/// better performance.
///
/// # Examples
///
/// The easiest way to use `FzOrederedMap` with a custom key type is to derive [`Eq`] and [`Ord`].
...
/// }
///
/// // Use a FzOrderedMap to store the vikings' health points.
/// let vikings = FzOrderedMap::new(vec![
    ...
    K: Ord + Eq,
{
    ...
    /// # use frozen_collections::FzOrderedMap;
    /// #
    /// let map = FzOrderedMap::new(vec![(1, 2), (3, 4)]);
    /// ```
    #[must_use]
...
impl<K, V> FzOrderedMap<K, V> {
    ...
    /// # use frozen_collections::FzOrderedMap;
    /// #
    /// let map = FzOrderedMap::new(vec![(1, "a".to_string())]);
    ///
    ...
    /// # use frozen_collections::FzOrderedMap;
    /// #
    /// let mut map = FzOrderedMap::new(vec![(1, "a".to_string())]);
    ///
    ...
    /// # use frozen_collections::FzOrderedMap;
    /// #
    /// let mut libraries = FzOrderedMap::new(vec![
